...
exports.addUser = (req, res) => {
    ...
    db.query(
        'INSERT INTO users (user_surname, user_name, user_patronymic, user_phone, user_login, user_password, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [surname, name, patronymic, phone, login, password, 1],  // is_active = 1
        (err, result) => {
...
exports.archiveUser = (req, res) => {
    const { id } = req.params;
    db.query("UPDATE users SET is_active = 0 WHERE user_id = ?", [id], (err) => {
        if (err) return res.status(500).json(err);
...
exports.restoreUser = (req, res) => {
    const { id } = req.params;
    db.query("UPDATE users SET is_active = 1 WHERE user_id = ?", [id], (err) => {
        if (err) return res.status(500).json(err);
...
exports.getUserDetails = (req, res) => {
    ...
    db.query(`
   SELECT u.*, 
       GROUP_CONCAT(ut.users_type_id) AS types,
       s.student_birthday, 
       s.student_parent_id AS parent_id,
       p.parent_user_id
FROM users u
LEFT JOIN users_types ut ON u.user_id = ut.users_id
...
exports.getActiveUsers = (req, res) => {
    db.query(`
        SELECT u.user_id, u.user_surname, u.user_name, u.user_patronymic, u.user_phone, 
               GROUP_CONCAT(ut.users_type_id) AS types
        FROM users u
        LEFT JOIN users_types ut ON u.user_id = ut.users_id
        WHERE u.is_active = 1
        GROUP BY u.user_id
        ORDER BY u.user_surname ASC;
    `, (err, results) => {
        if (err) return res.status(500).json(err);
        res.json(results.map(user => ({
            ...user,
...
exports.getArchivedUsers = (req, res) => {
    db.query(`
    SELECT u.user_id, u.user_surname, u.user_name, u.user_patronymic, u.user_phone, GROUP_CONCAT(ut.users_type_id) AS types
    FROM users u
    LEFT JOIN users_types ut ON u.user_id = ut.users_id
